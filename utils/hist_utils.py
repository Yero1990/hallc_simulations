...
def plot_1d_hist(fname_pattern='', pm_set=[], clr=[], scl_factor=[], title=r'', xlabel=r'', ylabel=r'', simc_flag=False):
    ...
    for i in enumerate(pm_set):
        ...
        if(simc_flag):
            plt.hist(x0, bins=nbins, weights = bin_cnt , range=[xmin, xmax], color='b', edgecolor='k', alpha=0.2 )
            plt.errorbar(x0, bin_cnt, yerr=stats_err,  linestyle='none', marker='o', color=clr[idx], ecolor=clr[idx], alpha = 0.4, ms=3, label = r'%d MeV/c' % (pm_set[idx]))            
        else:
            plt.hist(x0, bins=nbins, weights = bin_cnt , range=[xmin, xmax], color='b', edgecolor='k', alpha=0.2 )
...
def main():
    ...
    plot_1d_hist(fname_pattern='yield_Q2_pm%d.txt', pm_set=[700, 800, 900], clr=['b', 'g', 'r'], scl_factor=[45., 102., 204.], title=r'4-Momentum Transfer $Q^{2}$', xlabel=r'$Q^{2}$ (GeV/c)$^{2}$', ylabel=r'Yield', simc_flag=True)
