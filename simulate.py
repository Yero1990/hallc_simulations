...
if len(sys.argv) == 1:
    ...
    for ikin in input_list:
        ...
        run_simc_cmd = "./run_simc.sh %s " % (ikin)
        print(run_simc_cmd)
        ...
        run_fmake_cmd = "root -l -q -b \"fmake_tree.C(\\\"%s\\\")\" " % (ikin) 
        print(run_fmake_cmd)
    ...
    # optional: read input file as command line argument (assumed to be in infiles/, so only infput file nam  
    ifile = sys.argv[1]
    ...
    #define shell script command
    run_simc_cmd = "./run_simc.sh %s " % (ifile)
    print(run_simc_cmd)
    ...
    #define command to convert ASCII file to ROOTfile
    run_fmake_cmd = "root -l -q -b \"fmake_tree.C(\\\"%s\\\")\" " % (ifile) 
    print(run_fmake_cmd)
